...
contains
  ...
  subroutine step(f1, f2, model_padded2_dt2, dx, sources, sources_x,   &
    ...
    real, intent (in), dimension (nx_padded) :: model_padded2_dt2
    real, intent (in) :: dx
    ...
    if (even) then
      call step_inner(f2, f1, model_padded2_dt2, dx, sources,          &
        sources_x, step_idx, nx_padded, num_sources, source_len)
    else
      call step_inner(f1, f2, model_padded2_dt2, dx, sources,          &
        sources_x, step_idx, nx_padded, num_sources, source_len)
    end if
    end do
  ...
  subroutine step_inner(f, fp, model_padded2_dt2, dx, sources,         &
      sources_x, step_idx, nx_padded, num_sources, source_len)
    ...
    fp(i) = model_padded2_dt2(i) * f_xx + 2 * f(i) - fp(i)
    end do

    do i = 1, num_sources
    sx = sources_x(i) + 9;
    fp(sx) = fp(sx) + model_padded2_dt2(sx) * sources(i, step_idx)
    end do
  end subroutine step_inner

end module vfortran3
